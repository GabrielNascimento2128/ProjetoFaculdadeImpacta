...
from app.repositories import PetRepository, TutorRepository, FuncionarioRepository
from app.schemas import PetPersist, TutorPersist, FuncionarioPersist
...
def adicionar_funcionarios(funcionario: FuncionarioPersist, db: DB_dep, response: Response):
    db_func = Funcionario(nome=funcionario.nome, funcao=funcionario.funcao,
                          genero=funcionario.genero, telefone=funcionario.telefone,
                          email=funcionario.email, endereco_rua=funcionario.endereco_rua,
    ...
    return FuncionarioRepository.save(db, db_func)


@app.get("/funcionarios/{funci_id}", responses={HTTP_404_NOT_FOUND: {}})
def recuperar_funcionario_por_id(funci_id: int, db: DB_dep):
    funcionario_db = FuncionarioRepository.find_by_id(db, funci_id)

    if not funcionario_db:
        raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail="Funcionário não encontrado")
...
def atualiza_funcionario_por_id(funci_id: int, funcionario: FuncionarioPersist, db: DB_dep):
    ...
    funcionario_model =  Funcionario(id=funci_id, nome=funcionario.nome,
                                     funcao=funcionario.funcao, genero=funcionario.genero,
    ...
    FuncionarioRepository.save(db, funcionario_model)
...
def remove_funcionario_por_id(funci_id: int, db: DB_dep):
    success = FuncionarioRepository.delete_by_id(db, funci_id)
